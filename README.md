...
- **Validation**: required fields, positive amount, at most 2 decimal places, different source and destination, existing and active providers.
...
```
Controller → Service → DfspStrategyFactory → DfspStrategy → DfspClient (adapter) → DFSP over HTTP
                │
...
| `service` | Business validation, quote calculation, transfer orchestration |
| `strategy` | One strategy per DFSP, plus the factory that selects it |
...
3. `PaymentRequestValidator` checks that source and destination differ and that both providers exist and are `ACTIVE`.
4. `DfspStrategyFactory` returns the strategy for the **destination** provider.
...
| **Factory (selection)** | `DfspStrategyFactory` | Spring creates the strategies; the factory indexes them by provider code and returns the right one. Quote and transfer services share this lookup instead of each having a `switch`. It fails fast on duplicate or missing codes. |
...
A factory is **not** used for adapters: each strategy always uses the same adapter, so Spring injects it directly.

Adding a DFSP-C means adding a strategy, an adapter and a `providers` row. The quote and transfer services do not change.
...
```json
{ "status": 400, "error": "Bad Request", "message": "Source and destination provider cannot be the same", "fieldErrors": {} }
...
| 400 | Same source and destination; provider not found or inactive | `Provider not found: DFSP_X` |
...
| Suite | Tests | Covers |
|-------|-------|--------|
| `payment-router` | 65 | Validation, destination-fee pricing and rounding, strategy selection, adapter contracts for both DFSP formats, error JSON, CORS, repositories, persistence and pricing snapshot, and an end-to-end API test |
